...
import csv
from generate_synth_data import event_gen as generate 
...
for i in range(0,len(std),1): 
    var =str(int(std[i]))
    generate.randomized (beat,0, std[i],'random_std_'+ str(i))
#########################################
...
for i in range(0,len(num_values_to_select),1): 
    ...
    generate.matching(beat,num_values_to_select[i],'not_match_'+var)
...
for file_name, array in all_arrays.items():
    ...
    generate.plot_with_beats(beat,array,file_name)
